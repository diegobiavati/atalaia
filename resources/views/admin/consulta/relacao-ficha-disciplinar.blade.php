<form id="ficha-relatorio">
        ...
        <table class="table table-striped" style="margin: 60px 0 90px 0;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="javascript: void(0);" class="no-style" title="Visualizar FRAD do Aluno" onclick="abrirDisciplinarIndividual('{{route($rota)}}', '{{$aluno->id}}');"><i class="ion-android-clipboard" style="font-size: 22px;"></i></a></td>
                </tr>
                @endforeach
    ...
    <div style="margin-top: 24px;">
        <button id="btn-frad-geral" type="button" class="btn btn-primary" onclick="abrirDisciplinarGeral('{{route($rotaGeral)}}', '{{$idUeteCurso}}', '{{$anoFormacao->id}}');">Visualizar FRAD (Geral)</button>
    </div>
</form>
<script>
    function abrirDisciplinarIndividual(rota, aluno) {
        window.open(rota + '?aluno=' + aluno);
    }

    function abrirDisciplinarGeral(rota, idUeteCurso, idAnoFormacao) {
