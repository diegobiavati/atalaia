<form id="consulta-uete">
    ...
    <div style="margin-top: 24px;">
        <button id="btn-consulta-aluno" type="button" class="btn btn-primary" style="display: none;">Consultar Aluno(s)</button>
    </div>
</form>
...
<script>
    $(document).ready(function() {
        ...
        $('form#consulta-uete select.required_to_show_button').change(function() {
            $('form#consulta-uete select.required_to_show_button').each(function(index, element) {

                if ($(element).val() == 99) {
                    $('button#btn-consulta-aluno').slideUp(100);
                    return false;
                }
                $('button#btn-consulta-aluno').slideDown(100);
            });
        });

        $('button#btn-consulta-aluno').click(function(evt) {
            evt.stopImmediatePropagation(); //Não deixa duplicar os eventos

            var dados = $('#consulta-uete').serialize();
