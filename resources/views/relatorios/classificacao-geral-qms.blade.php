                    ...
                    <table style="border: 1px solid #000; border-collapse: collapse; margin: 32px auto; width: 90%; text-align: center;">
                        <tr style="background-color: #E6E6E6;">
                            ...
                            @foreach ($disciplinas as $key => $disciplina)
                                @if(is_numeric($key) && $disciplina['tfm'] == 'N')
                                ...
                                <tr> 
                                    ...
                                    @foreach ($disciplinas as $key => $disciplina)
                                        @if(is_numeric($key) && $disciplina['tfm'] == 'N')
                                            ...
                                            @foreach($aluno->data_demonstrativo as $data_demonstrativo)
                                                @if(isset($data_demonstrativo['disciplina_id']) && $disciplina->id == $data_demonstrativo['disciplina_id'])
                                    ...
                                    @php    
                                        $media_tfm = number_format($aluno->data_demonstrativo['avaliacoes_tfm']['media_tfm'], '3', ',', '');
                                    @endphp
