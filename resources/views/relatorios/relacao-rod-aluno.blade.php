<?php
    $url = ( (session()->get('login.omctID')) ? '/ajax/consulta-rod-aluno' : '/gaviao/ajax/consulta-rod-aluno');
?>
<form id="rodAluno">
    ...
    <div style="margin-top: 24px;">
        <button id="btn-rod-aluno" type="button" class="btn btn-primary" style="display: none;">Consultar Aluno(s)</button>
    </div>
</form>
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', 'select.required_to_show_button', function() {
            $('select.required_to_show_button').each(function(index, element) {
                ...
                $('button#btn-rod-aluno').slideDown(100);
            });
        });

        $('button#btn-rod-aluno').click(function() {
            var dados = $('#rodAluno').serialize();
