...
<div style="width: 960px; margin: 4px auto; page-break-after: always; text-align: center;">
    ...
    <h3 style="text-align: center; margin-top: -28px">ANÁLISE PARCIAL DO RESULTADO DA NOTA FINAL DO PERÍODO BÁSICO (NPB)</h3>
    <table style="border: 1px solid #000; border-collapse: collapse; margin: 32px auto; width: 100%;">
        <tr style="background-color: #E6E6E6;">
            <td style="border: 1px solid #000; padding: 6px; text-align: center;"><b>Ano de formação</b></td>
        ...
        <tr>
            <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$ano_selecionado}}</td>
            <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$omct}}</td>
            <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$npb_data['periodo']}}</td>
            <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$npb_data['efetivo']}}</td>
        </tr>
    </table>

    <div style="text-align: left">
        <p>
            <b>1. Média aritmética:</b> {{$npb_data['media_aritmetica']}}
        </p>    
        <p>
            <b>2. Limite da amplitude:</b>
            <ul>
                <li>Maior nota: {{ $npb_data['maior'] }}</li>
                <li>Menor nota: {{ $npb_data['menor'] }}</li>
                <li>Amplitude: {{ $npb_data['amplitude'] }}</li>
            </ul>
        </p>
        ...
        <table style="border: 1px solid #000; border-collapse: collapse; margin-top: 26px; width: 45%;">
            ...
            <tr>
                ...
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$npb_data['com_media']}}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($npb_data['com_media']*100)/$npb_data['efetivo'], 1, ',', '')}}</td>
            </tr>
            <tr>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">x<5</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$npb_data['sem_media']}}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($npb_data['sem_media']*100)/$npb_data['efetivo'], 1, ',', '')}}</td>
            </tr>
        </table>
                    ...
                    <table style="border: 1px solid #000; border-collapse: collapse; margin-top: 8px; width: 100%;">
                        ...
                        @foreach($npb_data['mencoes'] as $key => $item)
                            <tr>
                                ...
                                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($item*100)/$npb_data['efetivo'], 1, ',', '')}}</td>   
                            </tr>
                        @endforeach
                    </table>                    
                </td>
                        ...
                        @foreach($intervalo as $key => $item)
                            ...
                            @if(isset($notas_obtidas[$key]))
                                <tr>
                                    ...
                                    <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format((array_sum($notas_obtidas[$key])*100)/$npb_data['efetivo'], 1, ',', '')}}</td>  
                                </tr> 
                            @endif
        ...
        <p>
            <b>6. Alunos com nota abaixo de 5: </b> {{$npb_data['sem_media']}}  aluno(s)     
        </p>                          
