...
<div style="width: 960px; margin: 4px auto; page-break-after: always; text-align: center;">
    ...
    <div style="text-align: left">
        <p>
            <b>1. Média aritmética:</b> {{$taf_data['media_aritmetica']}}
        </p>    
        <p>
            <b>2. Limite da amplitude:</b>
            <ul>
                <li>Maior nota: {{ $taf_data['maior'] }}</li>
                <li>Menor nota: {{ $taf_data['menor'] }}</li>
                <li>Amplitude: {{$taf_data['amplitude']}}</li>
            </ul>
        </p>
        ...
        <table style="border: 1px solid #000; border-collapse: collapse; margin-top: 26px; width: 45%;">
            ...
            <tr>
                ...
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$taf_data['com_media']}}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($taf_data['com_media']*100)/$taf_data['efetivo'], 1, ',', '')}}</td>
            </tr>
            <tr>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">x<5</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{$taf_data['sem_media']}}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($taf_data['sem_media']*100)/$taf_data['efetivo'], 1, ',', '')}}</td>
            </tr>
        </table>
                    ...
                    <table style="border: 1px solid #000; border-collapse: collapse; margin-top: 8px; width: 100%;">
                        ...
                        @foreach($taf_data['mencoes'] as $key => $item)
                            <tr>
                                ...
                                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format(($item*100)/$taf_data['efetivo'], 1, ',', '')}}</td>   
                            </tr>
                        @endforeach
                    </table>                    
                </td>
                        ...
                        @foreach($intervalo as $key => $item)
                            ...
                            @if(isset($notas_obtidas[$key]))
                                <tr>
                                    ...
                                    <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{number_format((array_sum($notas_obtidas[$key])*100)/$taf_data['efetivo'], 1, ',', '')}}</td>  
                                </tr> 
                            @endif
        ...
        <p>
            <b>6. Alunos com nota abaixo de 5: </b> {{$taf_data['sem_media']}}  aluno(s)     
        </p>                          
