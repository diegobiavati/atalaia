...
class AjaxAdminController extends Controller
{
    ...
    public function GravarTafAluno(Request $request)
    {
        ...
        */
        ...
        if ($request->suficiencia_abdominal == 'NS') {
            $reprovado = 'S';
        } elseif ($request->suficiencia_abdominal == 'S' && $media >= 5) {
            $reprovado = 'N';
        } else {
            $reprovado = 'S';
        }
    ...
    public function GravarTafRecuperacaoAluno(Request $request)
    {
        ...
        if ((!isset($abdominal)) && $media >= 5) {
            ...
            $reprovado = 'S';
        } elseif ($abdominal == 'S' && $media >= 5) {
            $reprovado = 'N';
        } elseif ($abdominal == 'S' && $media == 0) {
            $reprovado = 'N';
            $media = 5;
        } else {
            $reprovado = 'S';
    ...
    public function AtualizarPeriodoEscolhaQMS(\App\Http\Controllers\OwnAuthController $ownauthcontroller, Request $request)
    {
        ...
        */
        ...
        } elseif (!@checkdate($mes_i, $dia_i, $ano_i)) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'Data de início invalida';
        } elseif (!@checkdate($mes_f, $dia_f, $ano_f)) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'Data final invalida';
        } elseif (!strtotime($request->hora_inicio)) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'Formato da hora inicial inválido.';
        } elseif (!strtotime($request->hora_fim)) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'Formato da hora final inválido.';
        } elseif (strtotime($ano_i . '-' . $mes_i . '-' . $dia_i . ' ' . $request->hora_inicio) >= strtotime($ano_f . '-' . $mes_f . '-' . $dia_f . ' ' . $request->hora_fim)) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'O início do período de escolha deve ser menor que a data/hora de término.';
        } elseif (isset($escolhas_registradas) && count($escolhas_registradas) > 1) {
            $data['status'] = 'err2';
    ...
    public function AdicionarPeriodoEscolhaQMS(Request $request)
    {
        ...
        */
        ...
        if (strlen($request->identificador) < 2) {
            ...
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'O início do período de escolha deve ser menor que a data/hora de término.';
        } elseif (isset($escolhas_registradas) && count($escolhas_registradas) > 0) {
            $data['status'] = 'err2';
            $data['erroPreencimento'] = 'O período de escolha informado não deve coincidir com nenhum outro período, portanto deverá ser único.';
        } else {
            $ano_corrente = AnoFormacao::orderBy('formacao', 'desc')->first();
    ...
    function dialogEditarAvaliacao(Request $request)
    {
        ...
        */
        ...
        if ($avaliacao->avaliacao_recuperacao == 0) {
            ...
            return view('ajax.avaliacao.view-adiciona-avaliacao-recuperacao', compact('avaliacao', 'disciplinas'));
    ...
    public function DialogChamadas(Request $request)
    {
        ...
        $data['peso'] = '<div style="margin: 14px auto; width: 70%; max-width: 380px;">
                            ...
                            <div style="float: right; border-bottom: 1px solid #ccc; width: 93%; margin-top: 4px; padding: 0 0 10px 6px; ">
                                <input class="no-style" style="width: 100%;" name="peso" type="text" value="' . ((isset($avaliacoes->peso)) ? $avaliacoes->peso : '') . '" maxlength="5" autocomplete="off" placeholder="Peso" />
                            </div>
                            <div class="clear"></div>
                        </div>';
        ...
        if ($disciplina->tfm == 'S') {
            ...
            $data['mostra'] = '<div style="margin: 14px auto; width: 70%; max-width: 380px;" data-toggle="tooltip" data-placement="right" title="Data da Mostra">
                                    ...
                                    <div style="float: right; border-bottom: 1px solid #ccc; width: 93%; margin-top: 4px; padding: 0 0 10px 6px; ">
                                        <input class="no-style data_mask" style="width: 100%;" name="data_mostra" type="text" value="' . ((isset($request->avaliacao)) ? FuncoesController::formatDateEntoBr($avaliacoes->data_mostra) : '') . '" maxlength="10" autocomplete="off" placeholder="Data da Mostra" />
                                    </div>
                                    <div class="clear"></div>
                                </div>
                                <div style="margin: 14px auto; width: 70%; max-width: 380px;" data-toggle="tooltip" data-placement="right" title="Limite de Dias do Pedido de Mostra">
                                    ...
                                    <div style="float: right; border-bottom: 1px solid #ccc; width: 93%; margin-top: 4px; padding: 0 0 10px 6px; ">
                                        <input class="no-style" style="width: 100%;" name="limite_dias_pedido" type="text" value="' . ((isset($request->avaliacao)) ? $avaliacoes->limite_dias_pedido : '') . '" maxlength="5" autocomplete="off" placeholder="Limite de Dias do Pedido de Mostra" />
                                    </div>
                                    <div class="clear"></div>
                                </div>
    ...
    public function MarcarUniverso(Request $request)
    {
        ...
        } elseif ($request->modo == 'ano-formacao') {
            $data['status'] = 'ok';
    ...
    public function OpcoesdeListagemSelecaoAlunos(\App\Http\Controllers\OwnAuthController $ownauthcontroller, Request $request)
    {
        ...
        } elseif ($ownauthcontroller->PermissaoCheck(1)) {
            $omcts = $request->omcts;
        } else {
