...
namespace App\Http\Controllers\EditableGrid;
...
class EditableGrid
{
    ...
    public function getXML($rows = false, $customRowAttributes = false, $encodeCustomAttributes = false, $includeMetadata = true)
    {
        ...
        if ($includeMetadata) {
            ...
            foreach ($this->columns as $name => $info) {
                ...
                if ($info['hidden']) {
                    $columnNode->setAttribute('hidden', 'true');
                }
    ...
    public static function mapToArray($map)
    {
        ...
        foreach ($map as $k => $v) {
            if (is_array($v)) {
                $array[] = array('label' => (string) $k, 'values' => self::mapToArray($v));
            } else {
                $array[] = array('value' => (string) $k, 'label' => $v);
            }
        }
